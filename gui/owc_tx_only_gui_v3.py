...
def normalize_line(line: str) -> str:
    line = line.strip()
    tokens = ["role ", "alive_tx ", "tx_frame ", "adc_mv ", "dac_mv ", "cmd_ok ", "cmd_err ", "tx_status "]
...
class OwcTxGui(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("OWC TX-ONLY Monitor - STM32F407 - v3")
    ...
    def _refresh_frame_widgets(self):
        ...
        self.frame_detail_var.set(
            ...
            f"full={hex_bytes(self.state.frame)}"
        )
        ...
        for idx, row in enumerate(rows):
            self.field_tree.insert("", "end", iid=f"row{idx}", values=row)
    ...
    def _redraw_wave(self):
        ...
        try:
            ...
            max_bits = 80
        bits = bits_from_bytes(self.state.frame)[:max_bits]
