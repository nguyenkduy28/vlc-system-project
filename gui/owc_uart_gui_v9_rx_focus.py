...
# -*- coding: utf-8 -*-
"""
OWC UART Monitor v9 RX Focus Demo
- RX-focused frame view: [Preamble] [Sync] [Length] [Payload] [Checksum]
- RX valid frame and RX error-frame viewer
- RX OOK waveform from received frame or error rx_bits
- Error frame capture tab

Requires:
    pip install pyserial matplotlib
Run:
    python owc_uart_gui_v9_rx_focus.py
"""
...
class AppData:
    ...
    last_err_bits: Dict[str, object] = field(default_factory=dict)
...
class OwcGui(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("OWC UART Monitor - STM32F407 - v9 RX Focus")
    ...
    def _build_ui(self):
        ...
        for tab, name in [
            (self.power_tab, "POWER"), (self.dac_tab, "DAC"), (self.txrx_tab, "RX DEMO"),
    ...
    def _build_txrx_tab(self):
        ...
        self.last_rx_var = tk.StringVar(value="Last valid RX: none")
        self.rx_status_var = tk.StringVar(value="RX status: waiting")
        ...
        self.rx_status_label = ttk.Label(summary, textvariable=self.rx_status_var, style="Warn.TLabel")
        self.rx_status_label.pack(side=tk.LEFT)

        # RX compact frame view
        frame_box = ttk.LabelFrame(self.txrx_tab, text="RX Frame View - tập trung xem board RX nhận được gì", padding=8)
        ...
        self.compact_labels: Dict[str, List[tk.Label]] = {"RX": []}
        self._build_compact_frame_widgets(self.rx_compact, "RX", [])

        self.rx_frame_detail_var = tk.StringVar(
            value="Frame hợp lệ sẽ hiển thị dạng: [AA AA AA] [D5] [LEN] [PAYLOAD] [CHECKSUM]. "
                  "Với 2 board khác nhau, GUI không dùng TX local để so sánh mặc định."
        )
        ttk.Label(frame_box, textvariable=self.rx_frame_detail_var, wraplength=1320).pack(anchor=tk.W, pady=(8, 0))

        # RX frame table + waveform
        ...
        fields_box = ttk.LabelFrame(left, text="RX Decoded Fields", padding=8)
        fields_box.pack(fill=tk.BOTH, expand=True)
        cols = ("field", "rx", "status")
        self.rx_field_tree = ttk.Treeview(fields_box, columns=cols, show="headings", height=8)
        for c, w in [("field", 110), ("rx", 270), ("status", 100)]:
            self.rx_field_tree.heading(c, text=c.upper())
            self.rx_field_tree.column(c, width=w, anchor=tk.W)
        self.rx_field_tree.tag_configure("ok", background="#eaffea")
        self.rx_field_tree.tag_configure("warn", background="#fff4d6")
        self.rx_field_tree.tag_configure("err", background="#ffd9d9")
        self.rx_field_tree.pack(fill=tk.BOTH, expand=True)

        self.rx_error_detail_var = tk.StringVar(value="Không có error frame chi tiết.")
        ttk.Label(fields_box, textvariable=self.rx_error_detail_var, wraplength=520).pack(anchor=tk.W, pady=(8, 0))
        ...
        ttk.Label(wave_top, text="RX waveform source:").pack(side=tk.LEFT)
        self.wave_source = tk.StringVar(value="Last valid RX")
        ttk.Combobox(
            wave_top,
            textvariable=self.wave_source,
            values=["Last valid RX", "Last error rx_bits"],
            width=18,
            state="readonly",
        ).pack(side=tk.LEFT, padx=(4, 12))
        ttk.Label(wave_top, text="View:").pack(side=tk.LEFT)
        self.wave_mode = tk.StringVar(value="Full frame")
        ...
        ttk.Spinbox(wave_top, from_=8, to=160, increment=8, textvariable=self.max_wave_bits, width=6).pack(side=tk.LEFT, padx=(4, 0))

        self.wave_fig = Figure(figsize=(8.5, 4.5), dpi=100)
        self.rx_wave_ax = self.wave_fig.add_subplot(111)
        ...
        metrics = ttk.LabelFrame(self.txrx_tab, text="RX Evaluation Metrics", padding=8)
        metrics.pack(side=tk.BOTTOM, fill=tk.X, padx=10, pady=(4, 8))
        self.metric_vars: Dict[str, tk.StringVar] = {}
        metric_items = [
            ("RX fps", "rx_frame_rate"),
            ("Goodput", "goodput_bps"),
            ("Frame success", "frame_success_rate"),
            ("PER", "frame_error_rate"),
            ("FW BER", "fw_payload_ber"),
            ("FW PER", "fw_per"),
    ...
    def _build_compact_frame_widgets(self, parent: ttk.Frame, prefix: str, frame: List[int]):
        ...
        for name in FIELD_NAMES:
            ...
            widget = tk.Label(parent, text=f"[{txt}]", bg=colors[name], fg="#111111", font=("Consolas", 12, "bold"), padx=8, pady=4, relief="groove")
    ...
    def _handle_line(self, line: str):
        ...
        if line.startswith("adc_mv"):
            ...
            ev = parse_err_bits_line(line)
            if ev is not None:
                ev["time"] = f"{time.time() - self.data.start_time:.1f}s"
                self.data.last_err_bits = ev
                self.data.last_error_text = (
                    f"err_bits frame_id={ev.get('frame_id')} mismatch_positions={ev.get('mismatch_positions', [])}"
                )
                self._add_error_event({
                    ...
                    "payload": "",
                    "raw": line,
                    "mismatch_positions": ",".join(str(p) for p in ev.get("mismatch_positions", [])),
                })
        elif line.startswith("err_frame"):
            ev = self._parse_err_frame(line)
            self.data.last_error_text = ev.get("raw", line)
            self._add_error_event(ev)
    ...
    def _refresh_compare_ui(self):
        ...
        # Update status only from RX perspective.
        rx = self.data.rx_frame
        for iid in self.rx_field_tree.get_children():
            self.rx_field_tree.delete(iid)

        if rx:
            fields = split_frame(rx)
            rows = [
                ("Preamble", hex_bytes(fields.get("Preamble", [])), "OK" if fields.get("Preamble") == TX_PREAMBLE else "WARN"),
                ("Sync", hex_bytes(fields.get("Sync", [])), "OK" if fields.get("Sync") == [TX_SYNC] else "WARN"),
                ("Length", hex_bytes(fields.get("Length", [])), f"{fields.get('Length', ['--'])[0] if fields.get('Length') else '--'} byte"),
                ("Payload", hex_bytes(fields.get("Payload", [])), "VALID"),
                ("Checksum", hex_bytes(fields.get("Checksum", [])), "VALID"),
            ]
            for i, row in enumerate(rows):
                tag = "ok" if row[2] in ("OK", "VALID") or "byte" in str(row[2]) else "warn"
                self.rx_field_tree.insert("", "end", iid=f"rx{i}", values=row, tags=(tag,))
            self.rx_status_var.set("RX status: valid frame decoded")
            self.rx_status_label.configure(style="Ok.TLabel")
            self.rx_frame_detail_var.set(f"Last valid RX frame: {hex_bytes(rx)}")
        else:
            self.rx_field_tree.insert("", "end", values=("RX", "--", "NO VALID FRAME"), tags=("warn",))
            self.rx_status_var.set("RX status: waiting for valid frame")
            self.rx_status_label.configure(style="Warn.TLabel")
            self.rx_frame_detail_var.set("Chưa có frame RX hợp lệ.")

        # Show last error details if available.
        if self.data.last_err_bits:
            positions = self.data.last_err_bits.get("mismatch_positions", [])
            self.rx_error_detail_var.set(
                f"Last err_bits: frame_id={self.data.last_err_bits.get('frame_id')} | "
                f"mismatch_positions={positions}. Dùng nguồn 'Last error rx_bits' để xem waveform lỗi."
            )
        elif self.data.last_error_text and self.data.last_error_text != "none":
            self.rx_error_detail_var.set(f"Last error: {self.data.last_error_text}")
        else:
            self.rx_error_detail_var.set("Không có error frame chi tiết.")

        self._redraw_rx_waveform()

    def _select_rx_bits(self) -> Tuple[List[int], List[int], str]:
        """Return bits, mismatch positions, and title for RX-focused waveform."""
        mismatch_positions: List[int] = []
        if self.wave_source.get() == "Last error rx_bits" and self.data.last_err_bits:
            rx_bits_str = str(self.data.last_err_bits.get("rx_bits", ""))
            bits = [1 if ch == "1" else 0 for ch in rx_bits_str if ch in "01"]
            mismatch_positions = [int(p) for p in self.data.last_err_bits.get("mismatch_positions", [])]
            title = f"RX bits from last error frame #{self.data.last_err_bits.get('frame_id')}"
            if self.wave_mode.get() == "Payload only" and len(bits) > 40:
                # Full frame layout: 3 preamble + sync + len = 5 bytes => payload starts at bit 40.
                bits = bits[40:40 + max(0, len(bits) - 48)]
                mismatch_positions = [p - 40 for p in mismatch_positions if p >= 40]
                title += " - payload region"
            return bits, mismatch_positions, title

        frame = self.data.rx_frame
        if self.wave_mode.get() == "Payload only":
            frame = split_frame(frame).get("Payload", []) if frame else []
            title = "RX valid payload OOK"
        else:
            title = "RX valid full-frame OOK"
        return bits_from_bytes(frame), [], title

    def _draw_ook_axis(self, ax, bits: List[int], title: str, mismatch_positions: List[int]):
        ax.clear()
        ax.set_title(title)
        ax.set_ylabel("Carrier gate")
        ax.set_xlabel("Time (µs), bit window = 10 µs")
        ax.grid(True)
        try:
            ...
            max_bits = 80
        bits = bits[:max_bits]
        mismatch_positions = [p for p in mismatch_positions if 0 <= p < max_bits]
        ...
        for i, bit in enumerate(bits):
            ...
            if i in mismatch_positions:
                ax.axvspan(t0, t0 + OOK_BIT_US, alpha=0.30)
            if bit == 1:
            ...
            if i in mismatch_positions:
                label += "*"
            ax.text(t0 + OOK_BIT_US/2, 1.08, label, ha="center", va="bottom", fontsize=7)
        if bits:
    ...
    def _redraw_rx_waveform(self):
        bits, mismatch_positions, title = self._select_rx_bits()
        self._draw_ook_axis(self.rx_wave_ax, bits, title, mismatch_positions)
