...
class OwcGui(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("OWC UART Monitor - STM32F407 - v6 RX Edge OOK")
    ...
    def _redraw_rx_ook_from_edges(self):
        """
        Realtime-ish RX OOK view reconstructed from UART log:
        last_edge_delta >= RX_EDGE_THRESHOLD -> bit 1 -> draw 10 cycles of carrier
        last_edge_delta <  RX_EDGE_THRESHOLD -> bit 0 -> draw OFF window

        This is not oscilloscope-grade sampling. It visualizes the decoded bit windows
        reported by firmware through UART alive logs.
        """
        self.rx_ook_ax.clear()
        self.rx_ook_ax.set_title(
            f"RX OOK from Edge Count: edge >= {RX_EDGE_THRESHOLD} -> bit 1, edge < {RX_EDGE_THRESHOLD} -> bit 0"
        )
        self.rx_ook_ax.set_xlabel("Bit window / time (µs, illustrative)")
        self.rx_ook_ax.set_ylabel("Carrier gate")
        self.rx_ook_ax.grid(True)
        ...
        for bit_index, bit in enumerate(bits):
            t0 = bit_index * OOK_BIT_US
            self.rx_ook_ax.axvline(t0, linewidth=0.5, linestyle="--", alpha=0.6)

            if bit == 1:
                # Draw 10 carrier cycles in this 10 us bit window.
                total_samples = int((OOK_BIT_US / OOK_CARRIER_PERIOD_US) * samples_per_carrier)
                for n in range(total_samples + 1):
                    t = t0 + n * (OOK_CARRIER_PERIOD_US / samples_per_carrier)
                    phase = (t - t0) % OOK_CARRIER_PERIOD_US
            ...
            # Label bit + edge value at top of each bit window.
        ...
        if bits:
            self.rx_ook_ax.axvline(len(bits) * OOK_BIT_US, linewidth=0.5, linestyle="--", alpha=0.6)

        if len(t_values) > 1:
            self.rx_ook_ax.step(t_values, y_values, where="post", label="RX OOK reconstructed")

        self.rx_ook_ax.set_ylim(-0.2, 1.35)
        self.rx_ook_ax.set_xlim(0, max(len(bits) * OOK_BIT_US, OOK_BIT_US))
        self.rx_ook_ax.legend(loc="upper right")
        self.rx_ook_canvas.draw_idle()
