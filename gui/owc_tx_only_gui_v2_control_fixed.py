...
def parse_hex_field(line: str, field: str) -> List[int]:
    # Captures: payload=55 A5 3C C3 checksum=FD
    # Captures only the hex list until the next key=value.
    match = re.search(rf"(?:^|\s){field}=([0-9A-Fa-fxX ]+?)(?:\s+[A-Za-z0-9_]+=|$)", line)
    if not match:
        return []

    # fallback split parser: keep only valid 1-byte hex tokens
    values: List[int] = []
